 ...
 *
 * @details
 * turns off the correct led as well as sets the static variable for the
 * next comp1 interrupt
 *
 * @note
 ...
 *
 * @details
 * turns on the correct led according to the static variable
 *
 * @note
 * This function is application specific and can be changed depending on
 * needs of program
...
void scheduled_letimer0_comp1_cb(void)
{
  ...
  if (LED_COLOR == 0) leds_enabled(RGB_LED_1,COLOR_RED,true);
  if (LED_COLOR == 1) leds_enabled(RGB_LED_1,COLOR_GREEN,true);
  if (LED_COLOR == 2) leds_enabled(RGB_LED_1,COLOR_BLUE,true);
}
