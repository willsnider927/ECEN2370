...
/***************************************************************************//**
 * @brief
 *   Interrupt hander for LETIMER0
 *
 * @details
 *   This routine will be called any time an interrupt occurs on LETIMER0.
 *   The interrupt will be checked and then the appropriate event will be scheduled.
 *
 * @note
 *   EFM_ASSERT statements check to make sure the interrupt flag is cleared before
...
void letimer_pwm_open(LETIMER_TypeDef *letimer, APP_LETIMER_PWM_TypeDef *app_letimer_struct){
  ...
  // ZZZ are values that you must specify for this PWM specific driver from the online HAL documentation
	letimer_pwm_values.bufTop = 0;		// Comp1 will not be used to load comp0, but used to create an on-time/duty cycle
	letimer_pwm_values.comp0Top = 1;		// load comp0 into cnt register when count register underflows enabling continuous looping
  ...
  /* Enable interrupts */
	...
	LETIMER_IntEnable(letimer, (app_letimer_struct->comp0_irq_enable * LETIMER_IEN_COMP0) | (app_letimer_struct->comp1_irq_enable * LETIMER_IEN_COMP1) |
...
void letimer_start(LETIMER_TypeDef *letimer, bool enable){
  if (enable && !(letimer->STATUS & LETIMER_STATUS_RUNNING)) // LETIMER going from inactive to active
  {
      sleep_block_mode(LETIMER_EM);
      LETIMER_Enable(letimer, enable);
      while(letimer->SYNCBUSY);
  }
  if (!enable && (letimer->STATUS & LETIMER_STATUS_RUNNING)) // LETIMER going from active to inactive
  {
