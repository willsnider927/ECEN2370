...
void ble_open(uint32_t tx_event, uint32_t rx_event){
  ...
  leuart_settings.rx_loc = LEUART_ROUTELOC0_RXLOC_LOC27;
  leuart_settings.tx_loc = LEUART_ROUTELOC0_TXLOC_LOC27;
  ...
  //leuart_settings.startframe_en = true;
  //leuart_settings.sigframe_en = true;

  leuart_open(BLE_LEUART,&leuart_settings);
}
...
bool ble_test(char *mod_name){
	...
	// ANSWER: Interrupts can be asynchronous, polling requires constant checking by the
	// ThunderBoard.
	// How does interrupts benefit the system for low energy operation?
	// ANSWER: It allows the use of energy modes not typically allowed in UART
	// communication by letting the processor do other tasks while waiting for
	// Uart interrupts
	// How does interrupts benefit the system that has multiple tasks?
	// ANSWER: the system can do other tasks while waiting for the interrupts due to being asynchronous
	...
	// Answer:  if the device doesn't sync, a wait for the syncbusy register would solve this
...
//	leuart_cmd_write(HM10_LEUART0, (LEUART_CMD_CLEARRX | LEUART_CMD_CLEARTX));
	...
	// DSD HM10 module?
	// ANSWER: Can't send AT commands while connected
